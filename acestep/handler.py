...
class AceStepHandler:
    ...
    def load_lora(self, lora_path: str) -> str:
        ...
        if not os.path.exists(config_file):
            return (
                f"❌ Invalid LoRA adapter: adapter_config.json not found in {lora_path}"
            )
        ...
        try:
            ...
            return (
                "❌ PEFT library not installed. Please install with: pip install peft"
            )
        ...
        try:
            ...
            if self.quantization is not None:
                ...
                try:
                    ...
                    raise ImportError(
                        "torchao is required for quantization but is not installed. Please install torchao to use quantization features."
                    )
            ...
            if not check_main_model_exists(checkpoint_path):
                logger.info(
                    "[initialize_service] Main model not found, starting auto-download..."
                )
            ...
            if not check_model_exists(config_path, checkpoint_path):
                logger.info(
                    f"[initialize_service] DiT model '{config_path}' not found, starting auto-download..."
                )
                success, msg = ensure_dit_model(
                    config_path, checkpoint_path, prefer_source=prefer_source
                )
                if not success:
                    return (
                        f"❌ Failed to download DiT model '{config_path}': {msg}",
                        False,
                    )
                logger.info(f"[initialize_service] {msg}")
            ...
            if os.path.exists(acestep_v15_checkpoint_path):
                ...
                try:
                    logger.info(
                        f"[initialize_service] Attempting to load model with attention implementation: {attn_implementation}"
                    )
                    ...
                    logger.warning(
                        f"[initialize_service] Failed to load model with {attn_implementation}: {e}"
                    )
                    if attn_implementation == "sdpa":
                        logger.info(
                            "[initialize_service] Falling back to eager attention"
                        )
                ...
                if not self.offload_to_cpu:
                    ...
                    if not self.offload_dit_to_cpu:
                        logger.info(
                            f"[initialize_service] Keeping main model on {device} (persistent)"
                        )
                        self.model = self.model.to(device).to(self.dtype)
                    else:
                ...
                if compile_model:
                    ...
                    if self.quantization is not None:
                        ...
                        if self.quantization == "int8_weight_only":
                            ...
                            raise ValueError(
                                f"Unsupported quantization type: {self.quantization}"
                            )

                        quantize_(self.model, quant_config)
                        logger.info(
                            f"[initialize_service] DiT quantized with: {self.quantization}"
                        )

                silence_latent_path = os.path.join(
                    acestep_v15_checkpoint_path, "silence_latent.pt"
                )
                if os.path.exists(silence_latent_path):
                    ...
                    raise FileNotFoundError(
                        f"Silence latent not found at {silence_latent_path}"
                    )
            else:
                raise FileNotFoundError(
                    f"ACE-Step V1.5 checkpoint not found at {acestep_v15_checkpoint_path}"
                )
            ...
            if os.path.exists(vae_checkpoint_path):
                ...
                raise FileNotFoundError(
                    f"VAE checkpoint not found at {vae_checkpoint_path}"
                )
    ...
    def _move_module_recursive(self, module, target_device, dtype=None, visited=None):
        ...
        for param_name, param in module._parameters.items():
            if param is not None and not self._is_on_target_device(
                param, target_device
            ):
                module._parameters[param_name] = param.to(target_device)
                if dtype is not None:
                    module._parameters[param_name] = module._parameters[param_name].to(
                        dtype
                    )
    ...
    def _recursive_to_device(self, model, device, dtype=None):
        ...
        if wrong_device_params and device != "cpu":
            logger.warning(
                f"[_recursive_to_device] {len(wrong_device_params)} parameters on wrong device, using state_dict method"
            )
            ...
            if model is not None:
                ...
                try:
                    ...
                    if param.device.type == "cpu":
                        logger.info(
                            f"[_load_model_context] Moving {model_name} to {self.device} (persistent)"
                        )
                        self._recursive_to_device(model, self.device, self.dtype)
                        if hasattr(self, "silence_latent"):
                            self.silence_latent = self.silence_latent.to(
                                self.device
                            ).to(self.dtype)
    ...
    def _parse_audio_code_string(self, code_str: str) -> List[int]:
        ...
        try:
            MAX_AUDIO_CODE = (
                63999  # Maximum valid audio code value (codebook size = 64000)
            )
            codes = []
            clamped_count = 0
            ...
            if clamped_count > 0:
                logger.warning(
                    f"[_parse_audio_code_string] Clamped {clamped_count} audio code value(s) to valid range [0, {MAX_AUDIO_CODE}]"
                )
            return codes
        except Exception as e:
            logger.debug(
                f"[_parse_audio_code_string] Failed to parse audio code string: {e}"
            )
            return []
    ...
    def _decode_audio_codes_to_latents(self, code_str: str) -> Optional[torch.Tensor]:
        ...
        with self._load_model_context("model"):
            ...
            indices = torch.tensor(
                code_ids, device=self.device, dtype=torch.long
            )  # [T_5Hz]
    ...
    def _dict_to_meta_string(self, meta_dict: Dict[str, Any]) -> str:
        ...
        timesignature = meta_dict.get(
            "timesignature", meta_dict.get("time_signature", "N/A")
        )
        keyscale = meta_dict.get(
            "keyscale", meta_dict.get("key", meta_dict.get("scale", "N/A"))
        )
    ...
    def build_dit_inputs(
        ...
        final_instruction = self._format_instruction(
            instruction or DEFAULT_DIT_INSTRUCTION
        )
        ...
        caption_input = SFT_GEN_PROMPT.format(
            final_instruction, actual_caption, parsed_meta
        )
        lyrics_input = self._format_lyrics(lyrics, actual_language)
        return caption_input, lyrics_input

    def _get_text_hidden_states(
    ...
    def extract_caption_from_sft_format(self, caption: str) -> str:
        try:
            ...
            logger.exception(
                "[extract_caption_from_sft_format] Error extracting caption"
            )
            return caption
    ...
    def prepare_seeds(self, actual_batch_size, seed, use_random_seed):
        ...
        if use_random_seed:
            ...
            actual_seed_list = [
                random.randint(0, 2**32 - 1) for _ in range(actual_batch_size)
            ]
                ...
                for s in seed_str_list:
                    if s == "-1" or s == "":
                        ...
                        try:
                            ...
                            logger.debug(
                                f"[prepare_seeds] Failed to parse seed value '{s}': {e}"
                            )
                            seed_list.append(-1)
    ...
    def _normalize_audio_code_hints(
        ...
        normalized = [
            hint if isinstance(hint, str) and hint.strip() else None
        ...
        return torch.stack(
            [
                torch.nn.functional.pad(
                    seq, (0, max_length - len(seq)), "constant", pad_value
                )
        ...
        if task_type == "text2music":
            ...
            if complete_track_classes and len(complete_track_classes) > 0:
                ...
                return TASK_INSTRUCTIONS["complete"].format(
                    TRACK_CLASSES=complete_track_classes_str
                )
            else:
    ...
    def process_reference_audio(self, audio_file) -> Optional[torch.Tensor]:
        ...
        try:
            ...
            logger.exception(
                "[process_reference_audio] Error processing reference audio"
            )
            return None
    ...
    def process_src_audio(self, audio_file) -> Optional[torch.Tensor]:
        ...
        try:
            ...
            raise ValueError(
                ...
                "Please ensure the file is a valid audio file (WAV, MP3, FLAC, OGG, etc.)."
            ) from e
        ...
        try:
            ...
            with torch.no_grad():
                ...
                attention_mask = torch.ones(
                    latents.shape[0], dtype=torch.bool, device=self.device
                )
                ...
                with self._load_model_context("model"):
                    ...
                    codes_string = "".join(
                        [f"<|audio_code_{idx}|>" for idx in indices_flat]
                    )

                    logger.info(
                        f"[convert_src_audio_to_codes] Generated {len(indices_flat)} audio codes"
                    )
                    return codes_string
            ...
            logger.exception(
                "[convert_src_audio_to_codes] Error converting audio to codes"
            )
            return error_msg
            ...
            if processed_src_audio is not None:
                if is_cover_task:
                    ...
                    padding_info_batch.append(
                        {"left_padding_duration": 0.0, "right_padding_duration": 0.0}
                    )
                    ...
                    left_padding_duration = (
                        max(0, -repainting_start) if repainting_start is not None else 0
                    )
        ...
        for wav in target_wavs_batch:
            if wav.shape[-1] < max_frames:
                ...
                padded_wav = torch.nn.functional.pad(
                    wav, (0, pad_frames), "constant", 0
                )
                padded_target_wavs.append(padded_wav)
            else:
                padded_target_wavs.append(wav)

        target_wavs_tensor = torch.stack(
            padded_target_wavs, dim=0
        )  # [batch_size, 2, frames]
        ...
        if can_use_repainting:
            ...
            if repainting_start is None:
                ...
                if processed_src_audio is not None:
                    adjusted_start = (
                        repainting_start
                        + padding_info_batch[0]["left_padding_duration"]
                    )
                ...
                for i in range(actual_batch_size):
                    if processed_src_audio is not None:
                        adjusted_start = (
                            repainting_start[i]
                            + padding_info_batch[i]["left_padding_duration"]
                        )
            ...
            if processed_src_audio is not None:
                ...
                if repainting_end is None or repainting_end < 0:
                    ...
                    adjusted_end = (
                        repainting_end + padding_info_batch[0]["left_padding_duration"]
                    )
                    repainting_end_batch = [adjusted_end] * actual_batch_size
            else:
    ...
    def _prepare_batch(
        ...
        audio_code_hints = self._normalize_audio_code_hints(
            audio_code_hints, batch_size
        )

        # Guard: refer_audios can be None when reference audio UI path didn't populate it (e.g. TEXT2MUSIC)
        if refer_audios is None:
            refer_audios = [
                [torch.zeros(2, 30 * self.sample_rate)] for _ in range(batch_size)
            ]
        ...
        for ii, refer_audio_list in enumerate(refer_audios):
            if isinstance(refer_audio_list, list):
                for idx, refer_audio in enumerate(refer_audio_list):
                    refer_audio_list[idx] = (
                        refer_audio_list[idx].to(self.device).to(torch.bfloat16)
                    )
        ...
        with torch.no_grad():
            ...
            with self._load_model_context("vae"):
                for i in range(batch_size):
                    ...
                    if code_hint:
                        logger.info(
                            f"[generate_music] Decoding audio codes for item {i}..."
                        )
                        ...
                        if decoded_latents is not None:
                            ...
                            frames_from_codes = max(
                                1, int(decoded_latents.shape[0] * 1920)
                            )
                    ...
                    if self.is_silence(current_wav):
                        ...
                        target_latent = self.silence_latent[
                            0, :expected_latent_length, :
                        ]
                    else:
                        # Encode using helper method
                        logger.info(
                            f"[generate_music] Encoding target audio to latents for item {i}..."
                        )
                        target_latent = self._encode_audio_to_latents(
                            current_wav.squeeze(0)
                        )  # Remove batch dim for helper
            ...
            wav_lengths = torch.tensor(
                [target_wavs.shape[-1]] * batch_size, dtype=torch.long
            )
            ...
            for latent in target_latents_list:
                ...
                if latent.shape[0] < max_latent_length:
                    ...
                    latent = torch.cat(
                        [latent, self.silence_latent[0, :pad_length, :]], dim=0
                    )
        ...
        instructions = self._normalize_instructions(
            instructions, batch_size, DEFAULT_DIT_INSTRUCTION
        )
        ...
        for i in range(batch_size):
            ...
            if repainting_start is not None and repainting_end is not None:
                start_sec = (
                    repainting_start[i] if repainting_start[i] is not None else 0.0
                )
                ...
                if end_sec is not None and end_sec > start_sec:
                    ...
                    end_latent = max(
                        start_latent + 1, min(end_latent, max_latent_length)
                    )
                    # Create mask: False = keep original, True = generate new
                    mask = torch.zeros(
                        max_latent_length, dtype=torch.bool, device=self.device
                    )
                ...
                chunk_masks.append(
                    torch.ones(max_latent_length, dtype=torch.bool, device=self.device)
                )
                spans.append(("full", 0, max_latent_length))
                # Determine task type from instruction, not from target_wavs
                # Only cover task should have is_cover=True
                instruction_i = (
                    instructions[i] if instructions and i < len(instructions) else ""
                )
                instruction_lower = instruction_i.lower()
                # Cover task instruction: "Generate audio semantic tokens based on the given conditions:"
                is_cover = (
        ...
        for i in range(batch_size):
            ...
            has_code_hint = audio_code_hints[i] is not None
            has_target_audio = has_code_hint or (
                target_wavs is not None and target_wavs[i].abs().sum() > 1e-6
            )
            ...
            if has_target_audio:
                ...
                if item_has_repainting:
                    ...
                    src_latent[start_latent:end_latent] = silence_latent_tiled[
        ...
        for i in range(batch_size):
            if audio_code_hints[i] is not None:
                ...
                logger.info(
                    f"[generate_music] Decoding audio codes for LM hints for item {i}..."
                )
                ...
                if hints is not None:
                    ...
                    if hints.shape[1] < max_latent_length:
                        ...
                        if (
                            pad_chunk.device != hints.device
                            or pad_chunk.dtype != hints.dtype
                        ):
                            pad_chunk = pad_chunk.to(
                                device=hints.device, dtype=hints.dtype
                            )
                        hints = torch.cat([hints, pad_chunk], dim=1)
                    elif hints.shape[1] > max_latent_length:
                        hints = hints[:, :max_latent_length, :]
                    precomputed_lm_hints_25Hz_list.append(
                        hints[0]
        ...
        # Extract caption and language from metas if available (from LM CoT output)
        # Fallback to user-provided values if not in metas
        actual_captions, actual_languages = self._extract_caption_and_language(
            parsed_metas, captions, vocal_languages
        )
        ...
        for i in range(batch_size):
            ...
            text_prompt = SFT_GEN_PROMPT.format(
                instruction, actual_caption, parsed_metas[i]
            )
        ...
        padded_text_token_idss = self._pad_sequences(
            text_token_idss, max_text_length, self.text_tokenizer.pad_token_id
        )
        padded_text_attention_masks = self._pad_sequences(
            text_attention_masks, max_text_length, 0
        )
        ...
        padded_lyric_attention_masks = self._pad_sequences(
            lyric_attention_masks, max_lyric_length, 0
        )
        ...
        if audio_cover_strength < 1.0:
            ...
            for i in range(batch_size):
                ...
                text_token_ids = text_inputs_dict.input_ids[0]
                non_cover_text_attention_mask = text_inputs_dict.attention_mask[
                    0
                ].bool()
        ...
        if audio_cover_strength < 1.0:
            assert padded_non_cover_text_input_ids is not None, (
                "When audio_cover_strength < 1.0, padded_non_cover_text_input_ids must not be None"
            )
            assert padded_non_cover_text_attention_masks is not None, (
                "When audio_cover_strength < 1.0, padded_non_cover_text_attention_masks must not be None"
            )
        # Prepare batch
        batch = {
            "keys": keys,
    ...
    def infer_refer_latent(self, refer_audioss):
        ...
        def _normalize_audio_2d(a: torch.Tensor) -> torch.Tensor:
            ...
            if a.dim() != 2:
                raise ValueError(
                    f"refer_audio must be 1D/2D/3D(1,2,T); got shape={tuple(a.shape)}"
                )
            if a.shape[0] == 1:
        ...
        for batch_idx, refer_audios in enumerate(refer_audioss):
            if len(refer_audios) == 1 and torch.all(refer_audios[0] == 0.0):
                ...
                for refer_audio in refer_audios:
                    ...
                    with torch.no_grad():
                        refer_audio_latent = self.tiled_encode(
                            refer_audio, offload_latent_to_cpu=True
                        )
                    # Move to device and cast to model dtype
                    refer_audio_latent = refer_audio_latent.to(self.device).to(
                        self.dtype
                    )
                    # Ensure 3D before transpose: [C, T] -> [1, C, T] -> [1, T, C]
                    if refer_audio_latent.dim() == 2:
                        refer_audio_latent = refer_audio_latent.unsqueeze(0)
                    refer_audio_latents.append(
                        _ensure_latent_3d(refer_audio_latent.transpose(1, 2))
                    )
                    refer_audio_order_mask.append(batch_idx)

        refer_audio_latents = torch.cat(refer_audio_latents, dim=0)
        refer_audio_order_mask = torch.tensor(
            refer_audio_order_mask, device=self.device, dtype=torch.long
        )
    ...
    def infer_text_embeddings(self, text_token_idss):
        with torch.no_grad():
            text_embeddings = self.text_encoder(
                input_ids=text_token_idss, lyric_attention_mask=None
            ).last_hidden_state
    ...
    def preprocess_batch(self, batch):
        ...
        with self._load_model_context("vae"):
            refer_audio_acoustic_hidden_states_packed, refer_audio_order_mask = (
                self.infer_refer_latent(batch["refer_audioss"])
            )
        if refer_audio_acoustic_hidden_states_packed.dtype != dtype:
            refer_audio_acoustic_hidden_states_packed = (
                refer_audio_acoustic_hidden_states_packed.to(dtype)
            )

        # step 4: chunk mask, N x T x d
        chunk_mask = batch["chunk_masks"]
        chunk_mask = (
            chunk_mask.to(device).unsqueeze(-1).repeat(1, 1, target_latents.shape[2])
        )
        ...
        with self._load_model_context("text_encoder"):
            ...
            non_cover_text_attention_masks = batch.get(
            ...
            if non_cover_text_input_ids is not None:
                ...
                non_cover_text_hidden_states = self.infer_text_embeddings(
                    non_cover_text_input_ids
                )
    ...
    def service_generate(
        ...
        if self.config.is_turbo:
            ...
            if infer_steps > 8:
                logger.warning(
                    f"[service_generate] dmd_gan version: infer_steps {infer_steps} exceeds maximum 8, clamping to 8"
                )
                infer_steps = 8
        ...
        audio_code_hints = (
            self._normalize_audio_code_hints(audio_code_hints, batch_size)
        ...
        outputs["target_latents_input"] = (
            target_latents  # Input target latents (before generation)
        )
        outputs["chunk_masks"] = chunk_mask
        outputs["spans"] = spans
        outputs["latent_masks"] = batch.get(
            "latent_masks"
        )  # Latent masks for valid length
    ...
    def tiled_decode(
        self, latents, chunk_size=512, overlap=64, offload_wav_to_cpu=True
    ):
        """
        ...
        if offload_wav_to_cpu:
            ...
            return self._tiled_decode_offload_cpu(
                latents, B, T, stride, overlap, num_steps
            )
        else:
    ...
    def _tiled_decode_offload_cpu(self, latents, B, T, stride, overlap, num_steps):
        ...
        first_end_idx = (
            first_audio_len - first_trim_end if first_trim_end > 0 else first_audio_len
        )
        ...
        for i in tqdm(range(1, num_steps), desc="Decoding audio chunks"):
            ...
            final_audio[:, :, audio_write_pos : audio_write_pos + core_len] = (
                audio_core.cpu()
            )
    ...
    def tiled_encode(
        self, audio, chunk_size=None, overlap=None, offload_latent_to_cpu=True
    ):
        """
        ...
        if offload_latent_to_cpu:
            result = self._tiled_encode_offload_cpu(
                audio, B, S, stride, overlap, num_steps, chunk_size
            )
        else:
            result = self._tiled_encode_gpu(
                audio, B, S, stride, overlap, num_steps, chunk_size
            )
    ...
    def _tiled_encode_gpu(self, audio, B, S, stride, overlap, num_steps, chunk_size):
        ...
        for i in tqdm(range(num_steps), desc="Encoding audio chunks"):
            ...
            audio_chunk = (
                audio[:, :, win_start:win_end].to(self.device).to(self.vae.dtype)
            )
    ...
    def _tiled_encode_offload_cpu(
        self, audio, B, S, stride, overlap, num_steps, chunk_size
    ):
        ...
        first_audio_chunk = (
            audio[:, :, first_win_start:first_win_end]
            .to(self.device)
            .to(self.vae.dtype)
        )
        with torch.no_grad():
        ...
        first_end_idx = (
            first_latent_len - first_trim_end
            if first_trim_end > 0
            else first_latent_len
        )
    ...
    def generate_music(
        ...
        if (
            self.model is None
            or self.vae is None
            or self.text_tokenizer is None
        ...
        actual_seed_list, seed_value_for_ui = self.prepare_seeds(
            actual_batch_size, seed, use_random_seed
        )
        ...
        try:
            ...
            if reference_audio is not None:
                ...
                if processed_ref_audio is not None:
                    ...
                    refer_audios = [
                        [processed_ref_audio] for _ in range(actual_batch_size)
                    ]
            else:
                refer_audios = [
                    [torch.zeros(2, 30 * self.sample_rate)]
            ...
            if src_audio is not None:
                ...
                if _has_audio_codes(audio_code_string):
                    logger.info(
                        "[generate_music] Audio codes provided, ignoring src_audio and using codes instead"
                    )
                else:
            ...
            is_repaint_task, is_lego_task, is_cover_task, can_use_repainting = (
                self.determine_task_type(task_type, audio_code_string)
            )

            repainting_start_batch, repainting_end_batch, target_wavs_tensor = (
                self.prepare_padding_info(
                    actual_batch_size,
                    processed_src_audio,
                    audio_duration,
                    repainting_start,
            ...
            progress(
                0.52, desc=f"Generating music (batch size: {actual_batch_size})..."
            )
            ...
            logger.info(
                "[generate_music] Model generation completed. Decoding latents..."
            )
            pred_latents = outputs[
                "target_latents"
            ]  # [batch, latent_length, latent_dim]
            time_costs = outputs["time_costs"]
            time_costs["offload_time_cost"] = self.current_offload_cost
            logger.debug(
                f"[generate_music] pred_latents: {pred_latents.shape}, dtype={pred_latents.dtype} {pred_latents.min()=}, {pred_latents.max()=}, {pred_latents.mean()=} {pred_latents.std()=}"
            )
            ...
            with torch.no_grad():
                with self._load_model_context("vae"):
                    ...
                    logger.debug(
                        f"[generate_music] Before VAE decode: allocated={self._memory_allocated() / 1024**3:.2f}GB, max={self._max_memory_allocated() / 1024**3:.2f}GB"
                    )

                    if use_tiled_decode:
                        logger.info(
                            "[generate_music] Using tiled VAE decode to reduce VRAM usage..."
                        )
                    ...
                    logger.debug(
                        f"[generate_music] After VAE decode: allocated={self._memory_allocated() / 1024**3:.2f}GB, max={self._max_memory_allocated() / 1024**3:.2f}GB"
                    )
            ...
            time_costs["total_time_cost"] = (
                time_costs["total_time_cost"] + time_costs["vae_decode_time_cost"]
            )
            ...
            logger.info(
                "[generate_music] VAE decode completed. Preparing audio tensors..."
            )
            if progress:
            ...
            logger.info(
                f"[generate_music] Done! Generated {len(audio_tensors)} audio tensors."
            )
            ...
            extra_outputs = {
                ...
                "target_latents": target_latents_input.detach().cpu()
                if target_latents_input is not None
                else None,
                "src_latents": src_latents.detach().cpu()
                if src_latents is not None
                else None,
                "chunk_masks": chunk_masks.detach().cpu()
                if chunk_masks is not None
                else None,
                "latent_masks": latent_masks.detach().cpu()
                ...
                "encoder_attention_mask": encoder_attention_mask.detach().cpu()
                if encoder_attention_mask is not None
                else None,
                "context_latents": context_latents.detach().cpu()
                if context_latents is not None
                else None,
                "lyric_token_idss": lyric_token_idss.detach().cpu()
                if lyric_token_idss is not None
                else None,
            }
    ...
    def get_lyric_timestamp(
        ...
        try:
            ...
            if seed is None:
                ...
                x0 = torch.randn(
                    x1.shape, generator=generator, device=gen_device, dtype=dtype
                ).to(device)
            ...
            header_ids = self.text_tokenizer.encode(
                header_str, add_special_tokens=False
            )
            ...
            if align_info.get("calc_matrix") is None:
                return {
                    ...
                    "token_timestamps": [],
                    "success": False,
                    "error": align_info.get(
                        "error", "Failed to process attention matrix"
                    ),
                }
    ...
    def get_lyric_score(
        ...
        try:
            ...
            encoder_attention_mask_in = torch.cat(
            ...
            attention_mask_in = torch.ones(
                2 * bsz, latent_length, device=device, dtype=dtype
            )
            past_key_values = None
